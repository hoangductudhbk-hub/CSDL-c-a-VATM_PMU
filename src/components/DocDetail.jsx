...
const extractPdfTextFull = async (buf) => {
  ...
  const maxPages = Math.min(pdf.numPages, 50) // đọc tối đa 50 trang
  let text = ''
...
const readFileFromUrl = async (url, fileName) => {
  const res = await fetch(url)
  const buf = await res.arrayBuffer()
  const ext = (fileName || '').split('.').pop().toLowerCase()
  if (ext === 'pdf') return extractPdfTextFull(buf)
  if (['doc','docx'].includes(ext)) return extractDocxText(buf)
  return ''
}
...
export default function DocDetail({ doc, onEdit, onClose }) {
  ...
  const handleDownload = async (e) => {
    ...
    try {
      ...
      a.href = url; a.download = fileName || 'tailieu'
  ...
  // ── Hỏi đáp sâu dùng bộ nhớ (không đọc lại file) ──
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.4)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100, padding:20 }}
      ...
      <div style={{ background:'#fff', borderRadius:14, padding:'24px 28px', width:'100%', maxWidth: showChat ? 720 : 560, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 8px 32px rgba(0,0,0,.15)', transition:'max-width .2s' }}>
        ...
        <div style={{ display: showChat ? 'grid' : 'block', gridTemplateColumns:'1fr 1fr', gap:20 }}>
                  ...
                  {memLoading ? (
                    ...
                    <>
                      ...
                      {memory.keywords?.length > 0 && (
                        <div style={{ marginTop:8, display:'flex', flexWrap:'wrap', gap:4 }}>
                          {memory.keywords.slice(0,8).map(kw => (
                    ...
                    <>
                      <div style={{ fontSize:12, color:'#92400e', marginBottom:10 }}>
                        🧠 <b>Chưa có bộ nhớ</b> — Phân tích 1 lần để hỏi đáp mãi mãi không tốn token đọc file!
                      </div>
                      {analyzeStep && (
          ...
          {showChat && memory && (
            <div style={{ display:'flex', flexDirection:'column', borderLeft:'0.5px solid #e5e4e0', paddingLeft:20, minHeight:400 }}>
              <div style={{ fontSize:13, fontWeight:600, color:'#0a2342', marginBottom:12 }}>
                💬 Hỏi đáp sâu về tài liệu
              </div>
              ...
              {/* Lịch sử chat */}
              <div style={{ flex:1, overflowY:'auto', display:'flex', flexDirection:'column', gap:8, marginBottom:10, maxHeight:340 }}>
                {chat.map((m, i) => (
                ...
                {aiLoading && (
                  <div style={{ display:'flex' }}>
                    <div style={{ padding:'8px 12px', borderRadius:10, fontSize:12, background:'#f5f5f3', color:'#888' }}>⏳ Đang tra cứu bộ nhớ...</div>
                  </div>
                )}
                <div ref={chatEndRef}/>
              </div>
