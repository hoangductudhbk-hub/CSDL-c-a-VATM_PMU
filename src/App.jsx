                    ...
                    {filtered.map(d => (
                      <tr key={d.id} onClick={() => { setDetailDoc(d); logViewDoc(d.code, d.subject, proj?.name) }}
                        ...
                        <td style={{ padding:'10px 12px', fontSize:15, maxWidth:320, position:'relative' }}>
                          {d.chunksCount > 0
                            ? <span style={{ display:'inline-flex', alignItems:'center', gap:3, fontSize:10, padding:'1px 6px', borderRadius:8, background:'#dcfce7', color:'#15803d', marginBottom:3, fontWeight:600 }}>✅ Đã đọc sâu · {d.chunksCount} chunks</span>
                            : d.extractedText
                              ? <span style={{ display:'inline-flex', alignItems:'center', gap:3, fontSize:10, padding:'1px 6px', borderRadius:8, background:'#fef9c3', color:'#854d0e', marginBottom:3 }}>⚡ Text cũ</span>
                              : null
                          }
